...
/**
 * FoodDeliverySystem is a Java Swing-based GUI application for managing food delivery orders.
 * <p>
 * Features:
 * <ul>
 *   <li>Add new food delivery orders with details such as Order ID, Customer Name, Food Item, Quantity, Address, and Delivery Person.</li>
 *   <li>Display all orders in a table with options to search by customer name.</li>
 *   <li>Delete selected orders from the list.</li>
 *   <li>Sample data is preloaded for demonstration purposes.</li>
 * </ul>
 * 
 * The application uses an {@link ArrayList} to store orders and a {@link JTable} to display them.
 * User interactions are handled via buttons for adding, searching, showing all, and deleting orders.
 * 
 * <b>Note:</b> This class requires the {@code FoodOrder} class to represent individual orders.
 * 
 * @author  (Your Name)
 * @version 1.0
 */
...
FoodDeliverySystem() {
        ...
        btnAdd.addActionListener(_ -> addOrder());
        btnShowAll.addActionListener( _-> showAllOrders());
        btnSearch.addActionListener(_-> searchOrders());
        btnDelete.addActionListener(_ -> deleteSelectedOrder());
